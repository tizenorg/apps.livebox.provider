...
struct packet *master_pd_destroy(pid_t pid, int handle, const struct packet *packet)
{
	...
	ret = packet_get(packet, "ssi", &arg.pkgname, &arg.id, &arg.info.pd_destroy.reason);
	if (ret != 3) {
		ErrPrint("Invalid packet\n");
		goto out;
	}
...
static struct method s_table[] = {
	...
	{
		.cmd = "lb_key_focus_in",
		.handler = provider_buffer_lb_key_focus_in,
	},
	{
		.cmd = "lb_key_focus_out",
		.handler = provider_buffer_lb_key_focus_out,
	},
	{
		.cmd = "pd_key_down",
	...
	},
	{
		...
		.handler = provider_buffer_lb_mouse_on_hold,
	},
	{
		.cmd = "lb_mouse_off_scroll",
		.handler = provider_buffer_lb_mouse_off_hold,
	},
	{
		.cmd = "pd_mouse_on_scroll",
		.handler = provider_buffer_pd_mouse_on_hold,
	},
	{
		.cmd = "pd_mouse_off_scroll",
		.handler = provider_buffer_pd_mouse_off_hold,
	},
	{
...
EAPI int provider_send_updated_NEW(const char *pkgname, const char *id, int w, int h, double priority, const char *content_info, const char *title, const char *icon, const char *name)
{
	...
	packet = packet_create_noack("updated", "ssiidsssss",
					pkgname, id, w, h, priority, content_info, title, safe_filename, icon, name);
	free(safe_filename);
	if (!packet) {
		ErrPrint("failed to build a packet\n");
		return LB_STATUS_ERROR_FAULT;
...
{
	return provider_send_updated_NEW(pkgname, id, w, h, priority, content_info, title, NULL, NULL);
}
...
EAPI int provider_send_key_status(const char *pkgname, const char *id, int status)
{
	...
	packet = packet_create_noack("key_status", "ssi", pkgname, id, status);
	if (!packet) {
		ErrPrint("Failed to build a packet\n");
		return LB_STATUS_ERROR_FAULT;
	}
...
EAPI int provider_send_request_close_pd(const char *pkgname, const char *id, int reason)
{
	...
	packet = packet_create_noack("close_pd", "ssi", pkgname, id, reason);
	if (!packet) {
		ErrPrint("Failed to build a packet\n");
		return LB_STATUS_ERROR_FAULT;
	}
